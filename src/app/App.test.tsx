...
describe('Trading 212 UI interactions', () => {
  ...
  it('toggles privacy mode and masks account totals and holding details', async () => {
    ...
    const eyeBtn = screen.getByRole('button', { name: '隐藏金额' })
    await user.click(eyeBtn)
    expect(screen.getByText('••••••••')).toBeTruthy()
    expect(screen.queryByText('€1,250.00')).toBeNull()
    const root = document.querySelector('.t212-native-app-root')
    expect(root?.classList.contains('hide-balances')).toBe(true)
