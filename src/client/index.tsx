...
function WorkspaceSwitch({ wide, placement = 'host' }: { wide: boolean; placement?: 'host' | 'overlay' }) {
  const open = useOpen()
  return <div data-placement={placement} className={`t212-workspace-switch ${wide ? 'is-wide' : ''} ${open ? 'is-dashboard' : 'is-dsh'}`} role="group" aria-label="工作区切换">
    <button className={!open ? 'active' : ''} type="button" aria-pressed={!open} onClick={() => setOpen(false)} aria-label="切换到 dsh">
      <span className="t212-switch-dsh">dsh</span>
    </button>
    <button className={open ? 'active' : ''} type="button" aria-pressed={open} onClick={() => setOpen(true)} aria-label="切换到 Trading 212 dashboard">
...
function PortfolioOverlay() {
  ...
  if (!open) return null
  return <div ref={dialogRef} className="t212-overlay" role="dialog" aria-label="Trading 212 投资组合" aria-modal="true" tabIndex={-1}>
    <div className="t212-overlay-switch-wrap" style={switchPosition}><WorkspaceSwitch wide placement="overlay" /></div>
    {frameFailed
      ? <div className="t212-frame-error" role="alert"><strong>无法打开 Trading 212</strong><span>请重启 dsh 后重试。</span><button type="button" onClick={() => { setFrameFailed(false); location.reload() }}>重新加载 dsh</button></div>
      : <iframe src="/trading212/" title="dsh Trading 212 投资组合" onError={() => setFrameFailed(true)} />}
  </div>
}
